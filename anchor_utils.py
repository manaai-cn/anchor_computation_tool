...
def compute_gt_annotations(
    ...
    """
    # what if size of anchor > annotations
    print('anchors shape: {}, ann shape: {}'.format(anchors.shape, annotations.shape))
    overlaps = compute_overlap(anchors.astype(np.float64), annotations.astype(np.float64))
    print('overlaps shape: {}'.format(overlaps.shape))
    # overlaps shape is a matrix, anchor x ann
    argmax_overlaps_inds = np.argmax(overlaps, axis=1)
    max_overlaps = overlaps[np.arange(overlaps.shape[0]), argmax_overlaps_inds]

    # assign "dont care" labels
    positive_indices = max_overlaps >= positive_overlap
    # maxoverlap > 0.5, then we make it show
